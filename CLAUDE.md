...
- **Patch notes**: when a change in this PR is user-facing, add a new fragment file to `src/patchNotes/entries/` before opening the PR — one JSON file per entry, `{ "message": "...", "date": "YYYY-MM-DD" }`, named after the change (e.g. `icon-action-buttons.json`). Keep the message short and written for someone who isn't reading the diff. Don't edit `src/utils/patchNotes.ts` or `src/utils/patchNotesData.ts` directly — the latter is generated from the fragments by `scripts/build-patch-notes.js` (runs automatically via `npm install`/`test`/`start`) and is gitignored. Using one file per entry, instead of everyone editing the same array, is what keeps parallel PRs from conflicting with each other here. Skip it for internal-only changes (refactors, tests, CI, tooling).
