...
| adding a calendar event, and who an event is with | `src/utils/eventPeople.ts` + `src/store/useEventPeopleStore.ts` + `presentEventCreate` in `src/utils/calendarSync.ts` — see `docs/arch/people.md`. The event goes through Apple's sheet into whatever calendar the user picks (Google included); the people link is app-only metadata keyed by occurrence, never an attendee, and stays on the device |
