...
`components/TaskItem.tsx` (4.4k), `utils/demoSeed.ts` (4.2k),
`store/useSettingsStore.ts` (3.8k), `screens/BackfillScreen.tsx` (3.6k).

Grep for the symbol and read the surrounding range; reading any of them end to end costs more
context than the rest of the task will. `docs/module-map.md` says which file owns what.

The suite is **340 test files**, and `npm test` runs all of them in well under a minute.
...
**Any `Modal` containing a focusable `TextInput` must call `Keyboard.dismiss()` on every path that can close it, or the app can freeze solid.** Closing a `Modal` while a `TextInput` inside it still holds native keyboard focus races the keyboard's own dismiss animation against the Modal's — the touch handler on whatever renders underneath (usually Today) gets stranded mid-handoff and stops responding to any tap at all, with no crash and no error to point at it. This is not theoretical: it shipped independently at least three times before being swept — `FoodLogEntrySheet`/`QuickAddModal` (#found first, "Fix Today freezing after a sheet closes with a focused text field"), a follow-up sweep across 27 more sheets in one commit (BarcodeScanSheet, CategoryEditor, CookbookEditor, EstimateMealSheet, GroceryItemSheet, LogMealPrompt, ProductSheet, RecipeCreateSheet, SuggestMealsSheet, and others), and then `CookModeSheet` (its "Ask about this step" field), missed by that very sweep despite matching the pattern exactly — reported as "logged a meal via a cook task, now Today is frozen." A later audit found nine *more* sites the sweep had missed: `GroceryAislesSheet`, `RemindMePicker`, `ProjectCategoriesSheet`, `CookbookChecklistSheet`, `ApplyTemplateSheet`, `NutritionSearchSheet`, `WhenPicker`, and the "Add existing task" and tag-detail modals in `ProjectDetailScreen`/`TagsScreen`. Grepping for `<Modal` and cross-referencing `TextInput` is the reliable way to find these — don't assume a file is fine because it's small or because the field commits immediately, since the freeze happens on *any* close while the field is merely focused, saved or not.

- **This applies to every `presentationStyle`** (`fullScreen`, `pageSheet`, and `transparent` popovers alike), not just the pageSheet swipe-dismiss case the rule above covers — that one is about data loss on an OS-driven gesture, this one is about a frozen screen on *any* close, including a plain button tap.
- **Find every way the Modal can close**, not just the obvious Cancel button: `onRequestClose`, a scrim tap (`SheetScrim`), a header back/close icon, a "Done"/"Save"/"Create" action, an `Alert` confirm callback, and any place a parent-supplied `onConfirm`/`onPick`/`onCreated` callback is invoked (the parent closing the sheet on success is still this sheet closing). `WhenPicker`'s bug was in its *confirm* paths (tapping a calendar day with the free-text field still focused), not its cancel path — check both directions.
- **Route every one of those through a single local function** (`close`/`cancel`/`handleDone`, matching whatever verb the sheet already uses) that calls `Keyboard.dismiss()` first, then does the real work — never call the raw `onClose`/`onCancel` prop, or `setState(false)` for an inline `Modal`, directly from more than one place. A missed call site is exactly how this keeps recurring.
- **Add the `Keyboard` import** (`react-native`) — its absence from a file with a `<Modal>` and a `TextInput` is itself the tell; every site this bug has shipped from was missing it entirely.
