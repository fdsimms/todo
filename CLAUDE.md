...
| a daily calorie figure worked out from a body | `src/utils/energyBudget.ts` — see `docs/arch/health-data.md`. Mifflin-St Jeor over fields the person typed, never read from Health; it **proposes** and the sheet's button is what writes `nutritionTargets.calorieKcal`. A missing field yields null, never an average, and `MIN_PROPOSED_KCAL` floors the suggestion without capping what the user may set |
...
`store/useTaskStore.ts` (8.3k), `db/database.ts` (5.5k), `types/index.ts` (5.5k),
`components/TaskEditor.tsx` (5.4k), `store/useGroceryStore.ts` (5.4k),
