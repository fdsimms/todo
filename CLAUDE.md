...
- **Hiding is the normal answer**, and it composes: `visible={addOpen && !scanOpen && !estimateOpen && !savedMealsOpen}`. Keep the *underlying* state set (`addOpen`, `pending`) so cancelling the raised sheet brings you back where you were — that "stays open behind" intent is fine, it just has to be expressed as state kept rather than as a second presented sheet.
...
- **Nesting is the other answer**, and the right one when the sheet below must keep its own typed state (`NutritionSearchSheet` inside `FoodLogEntrySheet`). A hidden sheet's children unmount once it finishes dismissing, so anything typed into it is gone when it comes back.
