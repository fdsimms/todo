...
/**
 * Ask the AI which tasks to move off today when the day's estimated workload
 * exceeds capacity. Prefers flexible, lower-priority tasks over urgent or
 * due-today ones, and already-focused tasks are never offered — the user
 * explicitly chose to keep those on today. Returns ids to defer to tomorrow.
 */
export async function suggestDeloadTasks(tasks: Task[], overMinutes: number): Promise<string[]> {
  const apiKey = useSettingsStore.getState().anthropicApiKey;
  if (!apiKey) throw new Error('No API key configured. Add your Anthropic API key in Settings.');
  ...
  const taskList = candidates.map(t => {
    const parts: string[] = [`"${t.title}"`];
    parts.push(`priority=${PRIORITY_NAMES[t.priority]}`);
    const mins = t.estimatedMinutes ?? effortToMinutes(t.effort) ?? 30;
    parts.push(`time=${formatDuration(mins)}`);
    if (t.dueDate) parts.push(`due=${t.dueDate.split('T')[0]}`);
    if (t.deadline) parts.push(`deadline=${t.deadline.split('T')[0]}`);
    if (t.category) parts.push(`category=${t.category}`);
    return `id:${t.id} ${parts.join(' ')}`;
  }).join('\n');
  ...
  const response = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      tools: [{
        name: 'deload',
        description: 'Return task ids to move off today to reduce the workload',
        input_schema: {
          type: 'object',
          properties: {
            task_ids: {
              type: 'array',
              items: { type: 'string' },
              description: 'Ids of tasks to move to tomorrow. Pick enough to free up roughly the target amount of time, without moving more than necessary.',
            },
          },
          required: ['task_ids'],
        },
      }],
      tool_choice: { type: 'tool', name: 'deload' },
      messages: [{
        role: 'user',
        content: [
          `Today's task list is overloaded by about ${formatDuration(overMinutes)}.`,
          `Pick tasks to move to tomorrow so today's workload drops by roughly that much. Prefer low-priority tasks, tasks without a due date or deadline today, and larger tasks that free up more time with fewer moves. Avoid urgent tasks and tasks due today unless there's no other choice.`,
          `Today: ${today}.`,
          '',
          taskList,
        ].join('\n'),
  ...
  const validIds = new Set(candidates.map(t => t.id));
  return toolUse.input.task_ids.filter(id => validIds.has(id));
}
