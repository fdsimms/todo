...
function seedGroceries(recipes: DemoRecipes): void {
  ...
  // ...and the store that closes that gap, which is what gives
  // ShoppingTripSheet's own suggestion card a second stop to name at all.
  // With Peanut butter and Cottage cheese below, Costco ends up carrying
  // three things Trader Joe's can't — a real gain the greedy walk picks up
  // on. One link short of that and the demo only ever shows the one-store
  // case, which is the common one but reads as the app having nothing to
  // say about a second stop.
