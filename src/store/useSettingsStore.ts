...
interface SettingsStore {
  ...
  //
  // A read switch beside `calendarReadEnabled` above and answering the same
  // question about a different framework: nothing is written, and the app only
  // ever looks. What it does *not* have is a permission field beside it, and
  // that is not an omission — HealthKit deliberately refuses to say whether a
  // read was allowed, so "you said no" and "there is nothing recorded" are one
  // state with no way back. This flag is therefore the user's whole answer
  // about the feature: off means nothing is read, on means the app asks and
  ...
  /**
   * Whether the person has already been told that Health is refusing the meals
   * this app tries to write, with nothing having gone in since.
   *
   * Bookkeeping rather than a preference, which is why no screen offers it. A
   * refused write is invisible by construction: the food log entry saves either
   * way, so the only sign is a meal that never reaches Health. Somebody who
   * turned "Log to Health" on and never granted sharing in Health's own sheet
   * loses every meal silently, and Settings' nutrition-write row only helps a
   * person who already suspects something.
   *
   * So the notice fires once rather than per meal, and a successful write
   * clears this so a *later* breakage is surfaced again instead of being
   * swallowed by having complained once years ago.
   */
  healthFoodWriteRefusalSeen: boolean;
  ...
  /**
   * Millilitres or fluid ounces, for the two places a volume of water is
   * stepped: the food log's own card and a task's "log to Health" amount.
   *
   * **Stored in ml either way**, the same split `weightUnit` keeps — this
   * decides what a stepper displays and steps in, never what `waterMl` holds.
   * A second conversion in a second place is how a figure lands wrong.
   *
   * A setting rather than the local state `TaskEditor` used to hold, because
   * the card is pressed several times a day and a preference that resets every
   * time the screen mounts is one nobody can actually have. Water is the one
   * nutrient worth this: nobody asks for a stepper in fluid ounces of sodium.
   */
  waterUnit: WaterUnit;
   ...
   * stored target itself — see `src/utils/waterExerciseBoost.ts`. Null until
   * somebody sets one, which is the shipping state.
   *
   * Both numbers are typed in, same discipline `weightGoal` above keeps: the
   * app proposes neither the threshold nor the amount, and nothing here ever
   * writes into `nutritionTargets.waterMl` itself, so the stored target a
   * person set never silently drifts.
   *
   * Kept out of DEFAULT_SETTINGS/resetToDefaults for the mechanical reason
  ...
  // two different permissions to give — "you may read Health" and "you may
  // write tasks from it" — and the pass requires both.
  //
  // `healthTaskCategory` is its own setting rather than a reuse of
  // `healthCategory`, even though both default to a category named "Health"
  // (addCategory is idempotent by name, so out of the box they share one
  // section). Clearing the reading's category means "don't show my step count
  // on Today", which is not the same instruction as "don't add health tasks",
  // and one setting could not tell those apart.
  healthTasks: boolean;
  healthTaskCategory: string | null;
   ...
   * which ones to write became a choice, so an install that predates the
   * setting keeps writing exactly what it always did. An empty array is a
   * real, distinct choice ("write nothing a meal states") rather than
   * "unset" — `parseHealthWriteNutrients` only falls back to the default when
   * the setting was never written at all. Read only by `logFoodEntryToHealth`;
   * it has no bearing on the two other writes (`Task.logHealthMetric`, which
   * already names one nutrient per task, and a recorded weight, which isn't a
   * nutrient at all).
   *
   * Kept out of DEFAULT_SETTINGS/resetToDefaults for the mechanical reason
   * foodLogPinnedNutrients is: it's an array, and String(value) doesn't
   * round-trip one — and a settings reset must not silently widen back out
...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
  ...
  initialize() {
    ...
    const waterUnit: WaterUnit = dbGetSetting('waterUnit') === 'flOz' ? 'flOz' : 'ml';
    ...
    // unreadable value salvages what it can. Same pair weatherRules draws.
    const storedHealthRules = dbGetSetting('healthRules');
    const healthRules = storedHealthRules ? parseHealthRules(storedHealthRules) : defaultHealthRules();
    ...
    //
    // This is the settings load, so essentially every feature adds a field to
    // it. As a single line it was a guaranteed conflict between any two
    // branches in flight at once. One per line was not enough on its own:
    // with no order, every feature appended to the same tail, so two branches
    // still inserted on the identical line and still conflicted. Sorted, a new
    // field lands at its own letter and two features touch different lines.
    //
    // settingsLoadSorted.test.ts enforces this, because nothing else can: a
  ...
  setHealthFoodWriteRefusalSeen(seen: boolean) {
    dbSetSetting('healthFoodWriteRefusalSeen', seen ? 'true' : 'false');
    set({ healthFoodWriteRefusalSeen: seen });
  },
  ...
  setWaterUnit(unit: WaterUnit) {
    dbSetSetting('waterUnit', unit);
    set({ waterUnit: unit });
  },
  ...
  setHealthTasks(on: boolean) {
    dbSetSetting('healthTasks', on ? 'true' : 'false');
    set({ healthTasks: on });
  },

  setHealthTaskCategory(category: string | null) {
    dbSetSetting('healthTaskCategory', category ?? '');
    set({ healthTaskCategory: category });
  },

  // Written whole, never patched — the list is one setting, same as
  // weatherRules and screenTimeRules. Kept out of DEFAULT_SETTINGS for their
  // mechanical reason: String(value) doesn't round-trip an array.
  setHealthRules(rules: HealthRule[]) {
    dbSetSetting('healthRules', serializeHealthRules(rules));
