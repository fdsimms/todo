...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
  ...
  initialize() {
    ...
    // One field per line, deliberately, and not to be re-joined.
    // This is the settings load, so essentially every feature adds a field to
    // it. As a single line that made it a guaranteed merge conflict between any
    // two branches in flight at once, with git unable to tell that the two sides
    // had added different things. One per line lets the line merge do its job.
    set({
