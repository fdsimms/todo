...
describe('isMissableMealPlanTask', () => {
  beforeEach(() => {
    jest.useFakeTimers();
    jest.setSystemTime(NOW);
  });
  ...
  it('returns false for a task not generated by the meal-plan slot generator', () => {
    const dueDate = new Date(2025, 5, 10, 0, 0, 0).toISOString();
    expect(isMissableMealPlanTask({ ...baseTask, generatedKind: null, dueDate })).toBe(false);
    expect(isMissableMealPlanTask({ ...baseTask, generatedKind: 'groceryUseUp', dueDate })).toBe(false);
  });

  it('returns true for a meal-plan task due today', () => {
    const dueDate = new Date(2025, 5, 10, 0, 0, 0).toISOString();
    expect(isMissableMealPlanTask({ ...baseTask, generatedKind: 'mealSlot', dueDate })).toBe(true);
  });

  it('returns false for a meal-plan task due on a future day — nothing to have missed yet', () => {
    const dueDate = new Date(2025, 5, 15, 0, 0, 0).toISOString();
    expect(isMissableMealPlanTask({ ...baseTask, generatedKind: 'mealSlot', dueDate })).toBe(false);
  });

  it('returns false for an already-completed meal-plan task', () => {
    const dueDate = new Date(2025, 5, 10, 0, 0, 0).toISOString();
    expect(isMissableMealPlanTask({ ...baseTask, generatedKind: 'mealSlot', dueDate, completed: true })).toBe(false);
  });
});
