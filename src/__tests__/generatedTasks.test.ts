...
describe('the registry', () => {
  ...
  it('shares one default category between planning the week and cooking it', () => {
    ...
    // pile up in the loose block above every section. Every kind is
    // categorized, calendarReview and supplyReorder included — each owns a
    // "File them under" setting of its own now, same as every other kind.
    expect(GENERATED_KIND_LIST.filter(s => s.categorized).every(s => !!s.defaultCategory)).toBe(true);
    expect(GENERATED_KIND_SPECS.calendarReview.categorized).toBe(true);
    expect(GENERATED_KIND_SPECS.supplyReorder.categorized).toBe(true);
  });
});
