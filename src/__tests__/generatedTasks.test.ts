...
describe('the registry', () => {
  ...
  it('gives every kind a category to file under, the nudge included', () => {
    ...
    expect(GENERATED_KIND_LIST.filter(s => s.categorized).map(s => s.kind))
      .toEqual(['mealSlot', 'groceryUseUp', 'pantryCheck', 'pantryReview', 'leftoverUseUp', 'mealPlanNudge', 'mealShortfall', 'projectReview', 'birthday', 'birthdayGift', 'reachOut', 'weather', 'screenTime', 'health', 'moodLog', 'moodNudge']);
  });
